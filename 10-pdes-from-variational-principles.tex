...
\begin{example}[Derivation of Minimal Surface Equation]
    ...
    \[
        z = h \pqty{x, y}
    \]
    for \(\pqty{x, y} \in \mathcal{D}\), with \(\mathcal{C}\) being the boundary of the graph, i.e. maps \(\partial \mathcal{D}\) to \(\mathcal{C}\). In particular, this means that \(h\) is fixed on \(\partial \mathcal{D}\).
...
\begin{example}[Deriving the Remaining Two Maxwell's Equations]
    ...
    \begin{subequations}
        ...
        \end{equation}
        \begin{equation}
            \curl{B} = \mu_0 \pqty{\vb{j} + \epsilon_0 \pdv{t} \vb{E}}
        \end{equation}
    \end{subequations}
